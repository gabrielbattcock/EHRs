...
oac_all_prescriptions <- data$therapy %>%
  inner_join(codes$oac, by="prodcode") 
ppi_all_prescriptions <- data$therapy %>%
  inner_join(codes$ppi, by="prodcode")  
h2ra_all_prescriptions <- data$therapy %>%
...
```{r}
par(mrow = c(1,3))
hist(oac_all_prescriptions$qty, xlim=c(0,200), breaks = 200)
hist(ppi_all_prescriptions$qty, xlim=c(0,200), breaks = 200)
hist(h2ra_all_prescriptions$qty, xlim=c(0,200), breaks = 200)
```
...
```{r}
oac_all_prescriptions %>% group_by(daily_dose) %>% 
  summarise(n = n()) %>%
  mutate(perc = prop.table(n))

h2ra_all_prescriptions %>% group_by(daily_dose) %>% 
  summarise(n = n()) %>%
  mutate(perc = prop.table(n))

ppi_all_prescriptions%>% group_by(daily_dose) %>% 
  summarise(n = n()) %>%
  mutate(perc = prop.table(n)) 
```
...
oac_all_prescriptions <- oac_all_prescriptions %>%
  mutate(duration=ifelse(daily_dose==0, NA, qty/daily_dose))
ppi_all_prescriptions <- ppi_all_prescriptions %>%
  mutate(duration=ifelse(daily_dose==0, NA, qty/daily_dose))
h2ra_all_prescriptions <-h2ra_all_prescriptions %>%
  mutate(duration=ifelse(daily_dose==0, NA, qty/daily_dose))
```
...
```{r}
par(mrow = c(1,3))
hist(oac_all_prescriptions$duration, xlim=c(0,200), breaks = 200)
hist(ppi_all_prescriptions$duration, xlim=c(0,200), breaks = 200)
hist(h2ra_all_prescriptions$duration, xlim=c(0,200), breaks = 200)
...
```{r}
table(oac_all_prescriptions$strength)
table(ppi_all_prescriptions$strength)
table(h2ra_all_prescriptions$strength)
...
it will be a cocktail with other drugs
...
ppi_group <- cohort %>% 
  filter(ppi == 1) %>% 
  select(ppi, patid, indexdate = eventdate)
...
```{r}
# Pick out all PPI prescriptions given to patients in the ppi_group on or after study entry and count per patient
num_subs_ppi<-ppi_group %>% 
  inner_join(ppi_all_prescriptions, by="patid") %>%
  filter(eventdate>=indexdate) %>%
  group_by(patid) %>%
  summarise(n_presc=sum(ppi))

# Tabulate
num_subs_ppi %>%
  mutate(group_n_presc=ifelse(n_presc<10, n_presc, 10)) %>%
  group_by(group_n_presc) %>%
  summarise(n = n())%>%
  mutate(perc = prop.table(n))

# Plot
hist(num_subs_ppi$n_presc, breaks = 0:max(num_subs_ppi$n_presc))
...
```{r}
# Pick out all PPI prescriptions given to patients in the ppi_group on or after study entry and count per patient
num_subs_h2ra<-h2ra_group %>% 
  inner_join(h2ra_all_prescriptions, by="patid") %>%
  filter(eventdate>=indexdate) %>%
  mutate(h2ra=1) %>% 
  group_by(patid) %>%
  summarise(n_presc=sum(ppi))
...
# Plot
hist(num_subs_h2ra$n_presc, breaks = 0:max(num_subs_h2ra$n_presc))
```
...
ppi_switchers <- ppi_group %>% 
  inner_join(h2ra_all_prescriptions, by = "patid") %>% 
  filter(eventdate>indexdate) %>% 
  arrange(patid, eventdate) %>% 
  filter(!duplicated(patid)) %>% 
  mutate(yrs_to_switch=as.numeric(eventdate-indexdate)/365.25) %>%
  rename(date_switch=eventdate) %>%
  select(patid, ppi, yrs_to_switch, date_switch)
```
...
```{r}
switchers <- rbind(ppi_switchers, h2ra_switchers)
```
...
enddates <- cohort %>% 
  ...
  mutate(enddate=pmin( #Minimum of ...
    as.Date("2017-04-17"), #...the study end date or ...
    pmin(tod, deathdate, na.rm = TRUE), #...the minimum of tod or deathdate.
    na.rm = TRUE),
...
names(switchers)
enddates <- enddates %>% 
  left_join(switchers[c("patid", "date_switch")], by="patid")
...
demographics <- cohort %>% 
  ...
  mutate(age=round(as.numeric(indexdate-as.Date(paste0(yob, "-06-15")))/365.25),
         calendarperiod=case_when(indexdate<as.Date("2000-01-01") ~ "1991-1999",
  ...
  select(patid, pracid, age, gender, imd_person, calendarperiod)
demographics
...
bmi_data <- data$clinical %>% 
  ...
  filter(between(as.numeric(indexdate)-as.numeric(bmi_date), 0, 5*365.25)) %>% # Keep measurements within 5 years
...
weight_data <- data$clinical %>% 
  ...
  filter(between(indexdate-weight_date, 0, 5*365.25)) %>% # Keep measurements within 5 years
...
height_data <- data$clinical %>% 
  ...
  filter(yoh-yob>=17)   %>% # Keep only post-17 measurements
...
analysis_data <- cohort %>% 
  ...
  # mutate(across(.cols = c("prior_diabetes", "recent_gerd"),
...
```{r}
saveRDS(analysis_data, file = "analysis_data.Rds")
```
